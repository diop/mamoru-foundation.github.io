...
Monitoring Transactions and Events: Sniffers continuously observe and process transactions, events, and state changes within the supported blockchains.

Applying Custom Rules: Sniffers apply user-defined SQL or WASM rules to detect incidents, allowing for highly customizable monitoring tailored to specific needs and requirements.

Real-time Incident Reporting: When an incident is detected, Sniffers quickly relay the information to the validation chain, enabling prompt and automated defensive actions.

Supporting Cross-Chain Analysis: Sniffers can operate across different chains and contracts, enabling the detection of complex threats and attack scenarios that span multiple blockchains.
